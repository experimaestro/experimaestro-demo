...
from experimaestro import RunMode, tag, copyconfig
...
def run_one_config(config: Configuration, tags: dict, gpulauncher, learn_builder, ds_mnist, tb) -> EvaluatedModel:
    ...
    Args:
        ...
        learn_builder: The pre-configured training task builder.
    ...
    # Compose the training task by binding the model to a copy of the shared learn task builder
    # to avoid mutating the original learn_builder across grid search permutations.
    learn_task = copyconfig(learn_builder) @ model
...
def run(helper: ExperimentHelper, cfg: Configuration):
    ...
    # Define the base training task configuration builder (template) once,
    # as the training parameters (data, epochs, lr, etc.) are the same across all sweeps.
    learn_builder = Learn.C(
        data=ds_mnist.train,
        epochs=cfg.epochs,
        n_val=cfg.n_val,
        lr=cfg.lr,
    ...
    for config, tags in zip(configurations, all_tags): 
        ...
        candidate = run_one_config(config, tags, gpulauncher, learn_builder, ds_mnist, tb)
