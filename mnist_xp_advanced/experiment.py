...
from experimaestro import RunMode, tag, GridSearch
...
def run(helper: ExperimentHelper, cfg: Configuration):
    ...
    # GridSearch: declarative way to sweep over parameters
    search = GridSearch()

    for n_layer in cfg.n_layers:
        for hidden_dim in cfg.hidden_dim:
            for kernel_size in cfg.kernel_size:
                with search.case(n_layers=tag(n_layer), hidden_dim=tag(hidden_dim), kernel_size=tag(kernel_size)):
    ...
    logging.info("Run `python -m mnist_xp_advanced.analyze` to inspect the results.")
